    ...
    return (
        ...
        >
            <div className="flex flex-col justify-center p-8">
                <div className="text-2xl my-8">{post.fields.title}</div>
                <div className="text-gray-500 text-sm">{post.fields.desc}</div>
...
export default function BlogPosts({ posts }) {
    ...
    return (
        <div className="mx-2 flex flex-col items-center my-8 max-w-screen-lg w-full w-2/3">
            <div className="text-gray-500 my-4">☕️ blogs & projects ⚒️</div>
            <div className="divide-y divide-fuchsia-300">
                {sorted.map((post) => (
                    <BlogPost key={post.id} post={post} />
                ))}
            </div>
            <div className="text-gray-500 my-4">More to come soon ...</div>
        </div>
    );
}
